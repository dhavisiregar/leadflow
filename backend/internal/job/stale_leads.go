...
func (j *StaleLeadJob) runWithCutoff(cutoff time.Time) {
	...
	// Excludes closed deals two ways: by status (the BRD's Won/Lost, which
	// covers every BRD-migrated tenant) and by legacy stage name (tenants
	// still on a custom stage set that literally has Won/Lost stages).
	err := j.DB.Table("leads").
		...
		Where("leads.status NOT IN ?", []string{"won", "lost"}).
