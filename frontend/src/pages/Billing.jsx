...
const PLANS = [
  {
    ...
    priceLabel: "Rp 0",
    priceSub: "forever",
    description: "Try LeadFlow for free",
    features: [
...
export default function Billing() {
  ...
  const handleDowngrade = async () => {
    ...
    try {
      ...
      if (msg === "leads exceed new plan limit") {
        ...
        setError(
          `Kamu punya ${d.leads} leads, melebihi batas ${d.plan_limit} leads di plan ini. Hapus beberapa leads dulu.`,
        );
      } else {
  ...
  const usagePct =
    maxLeads > 0 ? Math.min((leadsCount / maxLeads) * 100, 100) : 0;
      ...
      <div className="card p-5 mb-8 w-full max-w-xs">
        ...
        <div className="flex items-end justify-between mb-2">
          ...
          <span className="text-xs text-gray-400 dark:text-gray-500">
            {maxLeads > 0 ? `/ ${maxLeads} leads` : "/ unlimited"}
          </span>
        </div>
        {maxLeads > 0 && (
            ...
            >
              ...
              <ul className="space-y-1.5 flex-1 mb-5">
                {p.features.map((f) => (
                  <li
                    key={f.text}
                    className={`flex items-center gap-2 text-xs ${f.included ? "text-gray-700 dark:text-gray-200" : "text-gray-300 dark:text-gray-600"}`}
                  >
                    {f.included ? (
                      <Check
                        size={11}
                ...
                {p.note && (
                  <li className="text-[10px] italic text-gray-400 dark:text-gray-500 pl-[19px] mt-1">
                    {p.note}
                  </li>
                )}
              </ul>
          ...
          <div className="card w-full max-w-sm p-5">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h2 className="text-sm font-semibold text-gray-900 dark:text-white">
                Downgrade ke{" "}
                {PLANS.find((p) => p.key === confirmDowngrade)?.name}?
              </h2>
            </div>
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-5">
              Fitur yang tidak tersedia di plan ini akan dinonaktifkan. Aksi ini
              berlaku segera.
            </p>
