...
export default function Reports() {
        ...
        if (err.response?.status === 403) {
          setFeatureError(
            err.response?.data?.message ||
              "Reports feature is not available on your plan",
          );
        }
      })
  ...
  const hasData =
    data?.total_won > 0 || (data?.leads_created_by_month?.length ?? 0) > 0;
  ...
  return (
    <div className="p-4 sm:p-8 w-full">
      {featureError && <UpgradePrompt feature="Reports" />}
      {!featureError && (
        <>
          ...
          {loading ? (
            ...
            <div className="card p-16 text-center">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                Close your first deal to see reports
              </p>
            </div>
          ) : (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <StatCard
                  ...
                  value={
                    data.avg_days_to_close
                      ? `${Math.round(data.avg_days_to_close)}d`
                      : "—"
                  }
                  sub="created → won"
                  icon={Clock}
              ...
              {data.leads_created_by_month?.length > 0 && (
                <div className="card p-5">
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart
                      data={data.leads_created_by_month}
              ...
              {data.revenue_won_by_month?.length > 0 && (
                <div className="card p-5">
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart
                      data={data.revenue_won_by_month}
                      margin={{ top: 4, right: 8, bottom: 4, left: -16 }}
                    >
                      <CartesianGrid
                        strokeDasharray="3 3"
                    ...
                    {data.top_close_reasons.map((r, i) => {
                      ...
                      return (
                        <div key={r.reason} className="flex items-center gap-3">
                          ...
                          <span className="text-xs text-gray-700 dark:text-gray-200 w-28 truncate flex-shrink-0">
                            {r.reason}
                          </span>
                          ...
                          <span className="text-xs font-semibold text-gray-600 dark:text-gray-300 w-6 text-right flex-shrink-0">
                            {r.count}
                          </span>
                        </div>
                      );
