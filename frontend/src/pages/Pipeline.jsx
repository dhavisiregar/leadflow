...
import {
  getLeads, getStages, getContacts, getLead, getActivities, createActivity,
...
const ACT_ICONS = { call: Phone, email: Mail, meeting: MessageSquare, note: FileText }
const ACT_COLORS = { call: 'text-blue-500', email: 'text-green-500', meeting: 'text-purple-500', note: 'text-gray-400 dark:text-gray-500' }

function AgingBadge({ lead }) {
  const ref = lead.last_activity_at ? new Date(lead.last_activity_at) : new Date(lead.created_at)
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
      <div className="card w-full max-w-sm p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-gray-900 dark:text-white">Mark as {targetStage?.name}</h2>
          <button onClick={onCancel} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200"><X size={16} /></button>
        </div>
        <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">
          <span className="font-medium text-gray-700 dark:text-gray-200">{lead?.title}</span>
        </p>
...
function AddLeadModal({ stageId, stages, contacts, onClose, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
      <div className="card w-full max-w-sm p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-gray-900 dark:text-white">New Lead</h2>
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Contact</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Value (IDR)</label>
...
function LeadDetailPanel({ leadId, onClose, onUpdated }) {
  ...
  const isClosedStage = lead?.stage?.name === 'Won' || lead?.stage?.name === 'Lost'
    ...
    <div className="fixed inset-0 z-40 flex justify-end">
      ...
      <div className="w-full max-w-md bg-white dark:bg-gray-800 shadow-2xl flex flex-col border-l border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700 flex-shrink-0">
          <p className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide">Lead Detail</p>
        ...
        {loading ? (
          <div className="flex-1 flex items-center justify-center text-sm text-gray-400 dark:text-gray-500">Loading...</div>
        ) : (
            ...
            <div className="px-5 py-5 space-y-4">
              ...
              {editingField === 'title' ? (
                ...
                >
                  ...
                  <Pencil size={11} className="text-gray-300 dark:text-gray-600 group-hover:text-brand-400 flex-shrink-0 mt-1" />
                </h2>
              )}
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {editingField === 'value' ? (
                  ...
                  >
                    <DollarSign size={11} />
                    {lead.value > 0 ? formatIDR(lead.value) : <span className="italic text-gray-400 dark:text-gray-500">Add value</span>}
                    <Pencil size={9} className="text-gray-300 dark:text-gray-600" />
                  </button>
                )}
              </div>
              ...
              <div className="space-y-1.5 text-xs">
                {lead.contact?.name && (
                  <div className="flex items-center gap-2 text-gray-600 dark:text-gray-300">
                    <UserIcon size={13} className="text-gray-400 dark:text-gray-500 flex-shrink-0" />
                    <span>{lead.contact.name}</span>
                    {lead.contact.company && <span className="text-gray-400 dark:text-gray-500">· {lead.contact.company}</span>}
                  </div>
                )}
                {lead.owner?.name && (
                  <div className="flex items-center gap-2 text-gray-600 dark:text-gray-300">
              ...
              {isClosedStage && lead.close_reason && (
                <div className={`rounded-lg p-3 text-xs ${
                  ...
                  <p className="font-medium text-gray-700 dark:text-gray-200">
                    {lead.stage?.name === 'Won' ? 'Win reason' : 'Loss reason'}: {lead.close_reason}
                  </p>
                  {lead.close_note && <p className="text-gray-500 dark:text-gray-400 mt-0.5">{lead.close_note}</p>}
                </div>
              )}

              {/* Log activity */}
              <div className="border-t border-gray-100 dark:border-gray-700 pt-4">
                <p className="text-xs font-medium text-gray-700 dark:text-gray-200 mb-2">Log activity</p>
              ...
              <div className="border-t border-gray-100 dark:border-gray-700 pt-4 pb-4">
                <p className="text-xs font-medium text-gray-700 dark:text-gray-200 mb-3">
                  Activity{activities.length > 0 && <span className="text-gray-400 dark:text-gray-500 font-normal ml-1">({activities.length})</span>}
                </p>
                {activities.length === 0 ? (
                  <p className="text-xs text-gray-400 dark:text-gray-500 text-center py-6">No activities yet — log the first one above</p>
                ) : (
                    ...
                    {activities.map(act => {
                      ...
                      return (
                        <div key={act.id} className="flex gap-3">
                          <div className={`flex-shrink-0 mt-0.5 ${ACT_COLORS[act.type] || 'text-gray-400 dark:text-gray-500'}`}>
                            <Icon size={13} />
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5 capitalize">
                              {act.type} · {act.created_by?.name} · {new Date(act.created_at).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' })}
                            </p>
                          </div>
                        </div>
                      )
...
export default function Pipeline() {
  ...
  const handleDragEnd = (result) => {
    ...
    if (targetStage?.name === 'Won' || targetStage?.name === 'Lost') {
      setPendingMove({ leadId, newStageId, targetStage, lead: leads.find(l => l.id === leadId) })
      return
    }
  ...
  if (loading) return <div className="p-8 text-sm text-gray-400 dark:text-gray-500">Loading pipeline...</div>
    ...
    <div className="p-4 sm:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{leads.length} leads · drag to move between stages</p>
        </div>
        <button className="btn-primary flex items-center gap-2"
          onClick={() => { setActiveStage(stages[0]?.id); setShowModal(true) }}>
          <Plus size={14} /> <span className="hidden sm:inline">Add lead</span><span className="sm:hidden">Add</span>
        </button>
      </div>
              ...
              <div key={stage.id} className="flex-shrink-0 w-64">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded-full">{stageLeads.length}</span>
                  </div>
                  <button className="text-gray-400 dark:text-gray-500 hover:text-brand-600 transition-colors"
                ...
                <Droppable droppableId={String(stage.id)}>
                  {(provided, snapshot) => (
                    ...
                    >
                          ...
                          {(provided, snapshot) => (
                            ...
                            >
                              <div className="flex items-start justify-between gap-2">
                                <p className="font-medium text-gray-900 dark:text-white leading-snug">{lead.title}</p>
                              ...
                              {lead.contact?.name && (
                                <p className="text-gray-400 dark:text-gray-400 mt-1">{lead.contact.name}</p>
                              )}
                            </div>
                          )}
                        </Draggable>
                      ...
                      {stageLeads.length === 0 && (
                        <p className="text-xs text-gray-400 dark:text-gray-500 text-center py-4">Drop leads here</p>
                      )}
                    </div>
                  )}
                </Droppable>
      ...
      {showModal && (
        <AddLeadModal stageId={activeStage} stages={stages} contacts={contacts}
          onClose={() => setShowModal(false)} onCreated={handleCreated} />
      )}
      {editingLead && (
        <EditLeadModal lead={editingLead} stages={stages} contacts={contacts}
          onClose={() => setEditingLead(null)} onUpdated={handleUpdated} />
      )}
      {pendingMove && (
        <CloseReasonModal
          lead={pendingMove.lead} targetStage={pendingMove.targetStage}
          onConfirm={(reason, note) => { const { leadId, newStageId } = pendingMove; setPendingMove(null); doMove(leadId, newStageId, reason, note) }}
