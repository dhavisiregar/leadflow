...
// Owner-only: reassigns a lead to another tenant member (e.g. a different
// Sales rep). Kept separate from the generic Update so ownership changes are
// always explicit and validated, never a side effect of an edit form.
func (h *LeadHandler) Assign(c echo.Context) error {
	...
	if role != model.RoleOwner {
		return echo.NewHTTPError(http.StatusForbidden, "only the owner can reassign leads")
	}
