...
function DueDateBadge({ dueDate, isCompleted }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded whitespace-nowrap ${cls}`}
    >
      <Icon size={9} />
      {label}
    </span>
...
function PriorityBadge({ priority }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded whitespace-nowrap ${cfg.badge}`}
    >
...
function TaskRow({
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex items-center gap-1 flex-shrink-0 mt-0.5 transition-opacity ${hovered ? "opacity-100" : "opacity-0 pointer-events-none"}`}
      >
        <button
          onClick={openEdit}
...
const EMPTY = {
  all: {
    emoji: "📋",
    title: "No tasks yet",
    sub: "Add your first task to stay on top of your deals.",
  },
  today: {
    emoji: "🎉",
    title: "Nothing due today",
    sub: "Enjoy the breathing room!",
  },
  overdue: {
    emoji: "✅",
    title: "You're all caught up!",
    sub: "No overdue tasks.",
  },
  completed: {
    emoji: "⬜",
...
export default function Tasks() {
        ...
        if (err.response?.status === 403) {
          setFeatureError(
            err.response?.data?.message ||
              "Tasks feature is not available on your plan",
          );
        }
      })
  ...
  if (featureError) return <UpgradePrompt feature="Tasks" />;
              ...
              <div className="flex items-center gap-2 mb-2 px-1">
                ...
                {group.lead?.stage && (
                  <span
                    ...
                    style={{
                      backgroundColor: group.lead.stage.color || "#718096",
                    }}
                  >
